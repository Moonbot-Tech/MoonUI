...
impl InputState {
    ...
    pub fn set_multi_line(&mut self, multi_line: bool, cx: &mut Context<Self>) {
        ...
        self.mode = match &self.mode {
            InputMode::AutoGrow { .. } => InputMode::plain_text().multi_line(multi_line),
            _ => self.mode.clone().multi_line(multi_line),
        };
    ...
    pub fn set_auto_grow(&mut self, min_rows: usize, max_rows: usize, cx: &mut Context<Self>) {
        if matches!(
            &self.mode,
            InputMode::AutoGrow {
                min_rows: current_min,
                max_rows: current_max,
                ..
            } if *current_min == min_rows && *current_max == max_rows
        ) {
            return;
        }
        self.mode = InputMode::auto_grow(min_rows, max_rows);
    ...
    pub fn set_rows(&mut self, rows: usize, cx: &mut Context<Self>) {
        let changed = match &self.mode {
            InputMode::PlainText { rows: current, .. }
            | InputMode::CodeEditor { rows: current, .. } => *current != rows,
            InputMode::AutoGrow {
                max_rows: current_max,
                rows: current,
                ..
            } => *current != rows || *current_max != rows,
        };
        if !changed {
            return;
        }
    ...
    pub fn set_clean_on_escape(&mut self, clean_on_escape: bool, cx: &mut Context<Self>) {
        if self.clean_on_escape == clean_on_escape {
            return;
        }
        self.clean_on_escape = clean_on_escape;
        cx.notify();
    }
    ...
    pub fn sync_value(&mut self, value: impl Into<SharedString>, cx: &mut Context<Self>) {
        let text: SharedString = value.into();
        self.text = Rope::from(text.as_str());
        if let Some(diagnostics) = self.mode.diagnostics_mut() {
            diagnostics.reset(&self.text);
