...
impl MoonDataTableState {
    ...
    /// Record the latest rendered bounds for one header cell.
    fn record_header_bounds(&mut self, key: String, bounds: Bounds<Pixels>) {
        self.header_bounds.insert(key, bounds);
    }
    ...
    /// Snapshot rendered header widths before the first resize.
    fn header_widths(&self) -> Vec<(String, f32)> {
        self.header_bounds
            .iter()
            .map(|(key, bounds)| (key.clone(), f32::from(bounds.size.width)))
            .collect()
    }
}
...
impl MoonDataTable {
    ...
    pub fn ordered_columns(
        ...
        column_layout::ordered_columns(columns, state)
    }
    ...
    fn auto_width_columns(
        ...
        column_layout::auto_width_columns(columns, viewport_width, row_header_width, width_policy)
    }

    /// Apply header selection and sorting while preserving controlled-selection ownership.
    ...
    /// Open a root-owned context menu and clear retained context state on dismissal.
