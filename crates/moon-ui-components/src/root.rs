...
/// Controls whether the root should paint a background quad.
...
impl BackgroundPolicy {
    pub fn paints_fill(self) -> bool {
        !matches!(self, Self::NoFill)
    }

    pub fn fill_alpha(self, alpha: f32) -> f32 {
        match self {
            Self::Opaque => alpha,
            Self::Transparent | Self::NoFill => 0.0,
        }
    }

    pub fn apply<E>(self, element: E, color: u32, alpha: f32) -> E
    where
        E: Styled + FluentBuilder + Sized,
    {
        element.when(self.paints_fill(), |this| {
            this.bg(rgba(
                (color << 8) | ((self.fill_alpha(alpha) * 255.0).round() as u32),
            ))
        })
    }
}
...
impl Root {
    ...
    pub fn new(view: impl Into<AnyView>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        Self {
            ...
            background_policy: BackgroundPolicy::Opaque,
            background: None,
            text_color: None,
            font_family: None,
        }
    ...
    /// Set whether the root paints its background.
    pub fn background_policy(mut self, policy: BackgroundPolicy) -> Self {
        self.background_policy = policy;
        self
    }

    /// Override root background with a packed RGB value.
    pub fn background(mut self, rgb_hex: u32) -> Self {
        self.background = Some(rgb(rgb_hex).into());
        self
    }

    /// Override root text color with a packed RGB value.
    pub fn text_color(mut self, rgb_hex: u32) -> Self {
        self.text_color = Some(rgb(rgb_hex).into());
        self
    }

    /// Override root font family.
