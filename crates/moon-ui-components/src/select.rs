    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
    ...
    /// Change searchable mode at runtime.
    ...
    /// Replace the delegate (item data) without requiring a window handle.
    pub fn replace_items(&mut self, items: D, cx: &mut Context<Self>)
    where
        D: SearchableListDelegate + 'static,
    {
    ...
    /// Set open state from compatibility wrappers.
    pub fn set_open_for_moon(&mut self, open: bool, cx: &mut Context<Self>) {
