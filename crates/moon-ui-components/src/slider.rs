...
impl Slider {
    ...
    fn render_thumb(
        ...
        div()
            ...
            .flex_shrink_0()
            .corner_radii(radius)
            .size(MOON_SLIDER_THUMB_SIZE)
            .bg(thumb_color.opacity(if disabled { 0.42 } else { 1.0 }))
            .shadow(vec![box_shadow(
                px(0.),
                px(0.),
                px(14.),
                px(0.),
                thumb_color.opacity(if disabled { 0.14 } else { 0.72 }),
            )])
        ...
        div()
            ...
            .child(
                h_flex()
                    ...
                    .when(axis.is_horizontal(), |this| {
                        this.items_center().h(MOON_SLIDER_HEIGHT).w_full()
                    })
                    .when(axis.is_vertical(), |this| {
                        this.justify_center().w(MOON_SLIDER_HEIGHT).h_full()
                    })
                    .flex_shrink_0()
                    .child(
                        div()
                            ...
                            .relative()
                            .when(axis.is_horizontal(), |this| {
                                this.w_full().h(MOON_SLIDER_TRACK_HEIGHT)
                            })
                            .when(axis.is_vertical(), |this| {
                            ...
                            .child(
                                div()
                                    ...
                                    .bg(linear_gradient(
                                        if axis.is_horizontal() { 90.0 } else { 0.0 },
                                        linear_color_stop(accent.opacity(0.0), 0.0),
                                        linear_color_stop(accent.opacity(fill_alpha), 1.0),
                                    ))
                                    .rounded_full()
                                    .shadow(vec![box_shadow(
                                        px(0.),
                                        ...
                                        px(8.),
                                        px(0.),
                                        accent.opacity(if self.disabled { 0.10 } else { 0.34 }),
                                    )]),
                            )
                            .when(is_range, |this| {
...
mod tests {
    ...
    #[test]
    fn test_moon_slider_diffused_metrics_match_terminal_palette() {
        assert_eq!(MOON_SLIDER_HEIGHT, px(18.));
        assert_eq!(MOON_SLIDER_TRACK_HEIGHT, px(2.));
        assert_eq!(MOON_SLIDER_THUMB_SIZE, px(7.));
        assert_eq!(
            (MOON_SLIDER_THUMB_SIZE - MOON_SLIDER_TRACK_HEIGHT) * 0.5,
            px(2.5)
        );
    }

    #[test]
    fn test_slider_state_keeps_longbridge_range_capability() {
